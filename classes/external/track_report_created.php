...
/**
 * External service to track report creation.
 *
 ...
 */
class track_report_created extends external_api {
    /**
    ...
    public static function execute(string $reportname, bool $isaigenerated = false): array {
        ...
        try {
            ...
            if ($httpcode !== 200) {
                debugging('[Adeptus Insights] Report creation tracking failed - HTTP ' . $httpcode, DEBUG_DEVELOPER);
                return [
                    'success' => true,
                    'error' => false,
                    'message' => get_string('report_created_tracking_pending', 'report_adeptus_insights'),
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                debugging('[Adeptus Insights] Report creation tracking failed - invalid JSON', DEBUG_DEVELOPER);
                return [
                    'success' => true,
                    'error' => false,
                    'message' => get_string('report_created_tracking_pending', 'report_adeptus_insights'),
